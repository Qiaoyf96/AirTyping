...
class DepthToJETConverter: FilterRenderer {
    ...
    func render(pixelBuffer: CVPixelBuffer, imgBuffer: CVPixelBuffer) -> CVPixelBuffer? {
        if !isPrepared {
...
//        let hist = histogramBuffer.contents().bindMemory(to: Float.self, capacity: Int(self.jetParams.histogramSize))
        ...
        OpenCVWrapper.calcHistogram(for: pixelBuffer,bg: imgBuffer, to: outputPixelBuffer)
